...
function TestPlayerMovement:setUp()
...
function TestPlayerMovement:tearDown()
...
function TestPlayerMovement:test_direct_movement_succeeds_when_path_clear()
    -- Arrange
    ...
    DI.collision.is_walkable = function(opts) return true end
    DI.collision.is_blocked_by_entity = function(opts) return false end
    
    -- Act
    movement.handle(self.player, input, dt)
    
    -- Assert
    lu.assertEquals(self.player.pos.x, expected_pos.x, "Player should move right when path is clear")
    lu.assertEquals(self.player.pos.y, expected_pos.y, "Player y position should not change")
end

function TestPlayerMovement:test_wall_sliding_horizontal_when_diagonal_blocked()
    -- Arrange
    --[[
        Wall layout:
        .....
    ...
    lu.assertEquals(self.player.pos.y, 5, "Player y should not change when vertical path is blocked")
    lu.assertTrue(self.player.pos.x > 5, "Player should slide right when diagonal path is blocked but horizontal is open")
end

function TestPlayerMovement:test_wall_sliding_vertical_when_diagonal_blocked()
    -- Arrange
    --[[
        Wall layout:
        .....
    ...
    lu.assertEquals(self.player.pos.x, 5, "Player x should not change when horizontal path is blocked")
    lu.assertTrue(self.player.pos.y > 5, "Player should slide down when diagonal path is blocked but vertical is open")
end

function TestPlayerMovement:test_entity_sliding_when_walls_not_blockable()
    -- Arrange
    --[[
        Entity layout:
        .....
    ...
    DI.collision.is_blocked_by_entity = function(opts)
        ...
        return math.abs(opts.x - entity_pos.x) < 0.9 and 
               math.abs(opts.y - entity_pos.y) < 0.9
    end
    ...
    lu.assertTrue((self.player.pos - entity_pos):length() >= 0.9, "Player should have slid to avoid entity collision")
end

function TestPlayerMovement:test_slow_effect_reduces_movement_speed()
    -- Arrange
    local input = pos.new(1, 0) -- Move right
    local dt = 1
    ...
    -- Assert
    lu.assertEquals(self.player.pos.x, expected_pos.x, "Player should move at reduced speed when under slow effect")
    lu.assertEquals(self.player.pos.y, expected_pos.y, "Player y position should not change")
end

function TestPlayerMovement:test_no_movement_when_both_sliding_attempts_fail()
    -- Arrange
    --[[
        Layout (corner case):
        .....
    ...
    -- Assert
    lu.assertEquals(self.player.pos.x, start_pos.x, "Player x should not change when movement blocked in all directions")
    lu.assertEquals(self.player.pos.y, start_pos.y, "Player y should not change when movement blocked in all directions")
end

-- Run tests
if not arg then arg = {} end
if arg[1] == "test" then
    lu.LuaUnit.run('TestPlayerMovement')
end
